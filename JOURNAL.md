...
# Phase 10 — RANS Around Aerofoils

**Completed:** 2026-08-18
**Outcome:** 273/273 tests pass. Every input the simulation takes is reachable
from both the interface and the command line, five new field views, and the four
acceptance claims turned into tests - one of which failed and taught me something.

## 1. Scope

By this point the solver could already run a NACA section at Re = 10^6 with
k-omega SST. What it could not do was let anyone *change* most of what it was
solving. Turbulence intensity existed in a struct and was reachable from nowhere;
the length scale did not exist at all; viscosity was derived from the Reynolds
number with no way to state it directly; and of the nine quantities this phase
asks to be shown, four had no view.

So this phase is mostly plumbing - and the interesting part is what the plumbing
exposed.

## 2. Two inputs that were genuinely missing

**Turbulent length scale.** k comes from the intensity, and omega has to come
from somewhere. The implementation had only the eddy-viscosity ratio,
mu_t = R mu inverted for omega. That is the right way to state a clean tunnel,
where there is no physical eddy size to speak of. It is the wrong way to state
turbulence generated by a grid or a screen, where the eddy size is precisely
what is known, and the relation is

    omega = sqrt(k) / (beta*^(1/4) L)

Both are now available, with a positive length scale selecting the second.

**Viscosity as an input.** Phase 3 decided the Reynolds number is the input and
viscosity follows, and gave a good reason: two flows at the same Re over the same
shape are the same flow, so Re is the quantity that decides the answer. That
reasoning is still right, and it is not the only way anyone poses a problem.
Someone modelling a specific fluid at a specific speed knows mu and wants Re to
come out. `dynamicViscosityOverride` inverts the relationship without disturbing
the default.

## 3. The test that failed, and why it was the test

The four acceptance criteria are all of the form "changing X changes the flow",
which is exactly the sort of claim a solver can appear to satisfy while quietly
ignoring an input. Each became a test.

Three passed immediately. The Reynolds number one did not: friction drag fell
11% across two decades of Reynolds number, where flat-plate theory gives
Cf ~ Re^-0.2 and therefore about 60%.

The solver was not ignoring the Reynolds number. **My test was holding the wall
spacing fixed.** y+ = u_tau y / nu with u_tau ~ U sqrt(Cf/2) and Cf ~ Re^-0.2, so

$$y^+ \\propto y\\,Re^{0.9}$$

A mesh that puts the first cell at y+ = 0.1 at Re = 10^5 puts it at y+ = 10 at
Re = 10^7, and the low-Reynolds wall treatment - which has no wall function
behind it - is no longer valid there. The test was measuring the mesh going
stale, not the physics. Scaling the first layer as Re^-0.9 made it pass at the
theoretical rate.

That is worth more than a passing test, because it is a trap for anyone using
the program: **changing `--reynolds` alone produces wrong physics.** It is now in
the README as a warning with the numbers attached.

## 4. The colour range, learned twice

The five new views rendered, and three of them - k, omega and mu_t - came out
uniformly blank.

The cause is the same one as the C_f plot in Phase 5, which I evidently had to
learn a second time. These fields are almost entirely background with a thin,
violently intense feature in them: vorticity is enormous in the first cell off
the wall and essentially zero elsewhere, omega spans six decades over the same
distance, eddy viscosity lives entirely inside the boundary layer. Auto-ranging
on min and max hands those few cells the whole colour map and flattens the rest.

The range is now taken from the 2nd and 98th percentiles. The trimmed cells are
not hidden - the map clamps, so they saturate visibly at the ends - and every
view became legible, including the ones that were already working.

Worth noting what the corrected views *show*: a dark field with a bright rim
hugging the section. That is not a rendering failure; vorticity, k and mu_t
genuinely are confined to the boundary layer, and a domain that is twelve chords
of freestream ought to look mostly like freestream.

## 5. Status

**Verified by running it**

- 273/273 tests pass; zero warnings under `-DCFD_WARNINGS_AS_ERRORS=ON`.
- Camber produces lift at zero incidence, and 4412 lifts more than 2412.
- Friction drag falls with Reynolds number at close to Re^-0.2, once the wall
  spacing tracks it.
- A hundredfold change in freestream turbulence moves the friction drag.
- The length-scale and viscosity-override relations hold exactly.
- All nine requested quantities can be displayed.

**Not verified, and honest limitations**

- Everything from Phase 8 still stands: the grids are not converged, first-order
  convection dominates the error, and there is no transition model.
- The acceptance tests check *direction*, not magnitude. "More camber lifts more"
  is a much weaker statement than "the lift is right", and on these meshes the
  weaker statement is the one that can be made honestly.
- No test covers the field *views* themselves; they were checked by rendering and
  looking. A wrong scalar in a view would not fail the suite.

## 6. What I learned

- **"Changing X changes the flow" is a real test, not a formality.** One of the
  four failed, and the failure was more informative than the three passes - it
  found a way of using the program that produces plausible, wrong answers.

- **A failing test is a question, not a verdict.** The Reynolds test failed
  because the test was wrong, and the wrongness was itself the finding. Changing
  the assertion to make it pass would have thrown that away; so would changing
  the solver.

- **The same mistake arrives in different clothes.** Scaling a colour map to the
  extremes of a field with a thin intense feature is the identical error as
  scaling a C_f plot to its leading-edge singularity, five phases apart, and I
  did not recognise it until the pictures came out blank.

**Next:** a convection scheme that converges at second order, on explicit
instruction. It remains the largest source of error in every quantitative result
in this project.
