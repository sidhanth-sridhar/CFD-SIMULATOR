...
| 63 | **Cross-diffusion was clipped positive in the omega source term** | `CD_kw` needs two forms and the code had only one. As a *source* in the omega equation its sign is physical and a negative value is a real sink; inside F1's argument it is a *denominator* and has to be floored at 1e-10 to divide by. Menter clips only the second. Clipping both silently deleted every negative cross-diffusion contribution in the domain - which is most of the outer boundary layer, where grad(k) and grad(omega) point opposite ways. Found by auditing the implementation term by term against the published model, not by a failing test | The signed value goes to the source, the floored one to F1. Guarded by a test asserting that negative values exist somewhere in a solved field. Shifts C_l and C_d by under a percent |
